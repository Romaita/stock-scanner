...
def compute_weighted_return(closes: pd.Series) -> float:
    """Weighted price performance on DAILY closes, more weight on the recent
    quarter — approximates the IBD RS methodology (not their exact
    proprietary formula). Always fed daily-bar closes, regardless of the
    intraday cadence the rest of the scanner runs on."""
...
def compute_52w_range(daily_closes: pd.Series, current_price: float):
    """Returns (pct off 52-week high, pct above 52-week low), using DAILY
    closes for the year-long window. Positive off_high means below the
    high; e.g. -5.0 means 5% below the 52-week high. current_price is the
    latest intraday price, so this compares live price against the daily
    52-week range."""
...
def main():
    ...
    for ticker, yf_symbol in zip(TICKERS, yf_symbols):
        try:
            ...
            price = float(closes_5m.iloc[-1])                     # latest intraday price
            prev_bar_close = float(closes_5m.iloc[-2])
            chg = round((price - prev_bar_close) / prev_bar_close * 100, 1)  # change vs prior 5m bar
            ...
            # relative strength vs SPY: weighted multi-period DAILY return, stock minus benchmark
